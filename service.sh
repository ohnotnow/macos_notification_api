...
PLIST_NAME="com.ohnotnow.notification-server-python.plist"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PLIST_DEST="$HOME/Library/LaunchAgents/$PLIST_NAME"
LOG_DIR="$HOME/Library/Logs/notification-server-python"
...
# Find uv binary - use explicit path for launchd reliability
UV_PATH=$(which uv)
...
<dict>
    <key>Label</key>
    <string>com.ohnotnow.notification-server-python</string>
...
case "$1" in
    ...
    status)
        if launchctl list | grep -q "com.ohnotnow.notification-server-python"; then
            echo "Service is running"
            echo "  Configured: http://${SERVER_HOST}:${SERVER_PORT}"
            launchctl list | grep "com.ohnotnow.notification-server-python"
            echo ""
